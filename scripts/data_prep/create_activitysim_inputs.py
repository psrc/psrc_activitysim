...
# Set  directories for all inputs 
...
standard_parcel_path = r"R:\e2projects_two\SoundCast\Inputs\dev\landuse\2018\vers2_july2020\parcels_urbansim.txt"
...
# If true, create additional columns from buffered land use file instead of standard parcel file
use_buffered_parcels = True

zone_type = "MAZ"

lu_aggregate_dict = {
    "hh_p": "sum",      # total households
    "emptot_p": "sum",  # total employees
    "stugrd_p": "sum",  # total grade school students
    "stuhgh_p": "sum",  # total high school students
    "stuuni_p": "sum",  # total university students
    "empedu_p": "sum",  # total educational employees: health, education, and recreational
    "empfoo_p": "sum",  # total retail trade employees: retail trade
    "empgov_p": "sum",  # total government employees: other employment
    "empind_p": "sum",  # total industrial employees: manufacturing, wholesale trade, and transport
    "empmed_p": "sum",  # total medical employees: health, educational, and recreational
    "empofc_p": "sum",  # total other office employees: financial and professional services
    "empret_p": "sum",  # total retail employees: retail
    "empsvc_p": "sum",  # total other service employees: other employment
    "empoth_p": "sum",  # total construction, mining, and other employees: natural resources
    "hh_1": "mean",     # average number of households within 1/2 mile buffer
    "hh_2": "mean",     # average number of household within 1 mile buffer
    "emptot_1": "mean", # average number of employees within 1/2 mile buffer
    "emptot_2": "mean", # average number of employees within 1 mile buffer
    "empret_1": "mean", # average number of retail employees within 1/2 mile buffer
    "empsvc_1": "mean", # average number of service employees within 1/2 mile buffer
    "sfunits": "sum",   # total single family units
    "mfunits": "sum",   # total multifamily units
    "TAZ": "first",     # TAZ ID
}
...
def process_landuse(df_psrc, df_psrc_person, zone_type, use_buffered_parcels):
    ...
    if use_buffered_parcels:
        df_parcel = pd.read_csv(buffered_parcel_path, delim_whitespace=True)
    else:
        df_parcel = pd.read_csv(standard_parcel_path, delim_whitespace=True)
    ...
    if use_buffered_parcels:
        df_lu = df_parcel.groupby(zone_type, as_index=False).agg(lu_aggregate_dict)
    else:
    ...
    df_lu.rename(
        columns={
            "hh_p": "TOTHH",        # total households
            "emptot_p": "TOTEMP",   # total employment
            "stugrd_p": "GSENROLL", # students: grade school
	...
	# Distance to transit - Get closest for now
    if use_buffered_parcels:
...
df_lu = process_landuse(df_psrc, df_psrc_person, zone_type, use_buffered_parcels)