...
def park_and_ride_skims(my_project, sc_tod, spec_dir, results_dict, asim_tod):
    ...
    for asim_name, matrix_name in matrix_dict.items():
        print (asim_name)
        try:
            matrix_value = emmeMatrix_to_numpyMatrix(matrix_name, my_project.bank, 'uint16', 1)
    ...
    pnr_total_wait_access = emmeMatrix_to_numpyMatrix('pnr_total_wait_access', my_project.bank, 'uint16', 1)
    pnr_initial_wait_access = emmeMatrix_to_numpyMatrix('pnr_initial_wait_access', my_project.bank, 'uint16', 1)
    ...
    pnr_total_wait_egress= emmeMatrix_to_numpyMatrix('pnr_total_wait_egress', my_project.bank, 'uint16', 1)
    pnr_initial_wait_egress = emmeMatrix_to_numpyMatrix('pnr_initial_wait_egress', my_project.bank, 'uint16', 1)
...
def ferry_assignment(my_project, ferry_spec_dir, class_name):
    ...
    # Skims:
    skim_ferry = my_project.m.tool("inro.emme.transit_assignment.extended.matrix_results")

    skim_list = ['b', 'c', 'f', 'r', 'components']
    for skim in skim_list:
        skim_ferry(json.load(open(ferry_spec_dir/f'asim_ferry_skim_{skim}.json')), class_name=class_name)

def all_transit_assignment(my_project, transit_spec_dir, class_name, matrix_list, results_dict, asim_tod):
    ...
    skim(json.load(open(transit_spec_dir/'all_transit_skim_spec.json')), class_name=class_name)
    # Export
...
def create_supplemental_skims(my_project: EmmeProject, skims_file_path: Path, spec_dir: Path, time_period_lookup: dict):
    ...
    for asim_tod, sc_tod in time_period_lookup.items():
        ...
        if asim_tod != 'NI':
            ...
            all_transit_assignment(my_project, all_transit_dir, all_transit_class_name, matrix_list, results_dict, asim_tod)
