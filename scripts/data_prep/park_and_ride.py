...
def two_leg_trip(my_project, spec_dir):
    ...
    # DRV_TRN_WLK ()
    two_leg_trip(
        tour_demand=my_project.bank.matrix("pnr_demand_access"),
    ...
    # WLK_TRN_DRV (EGRESS)
    two_leg_trip(
        tour_demand=my_project.bank.matrix("pnr_demand_egress"),
...
def run_park_and_ride(
    ...
    pnr_capacities = pd.read_csv(
        project_path / "inputs/scenario/networks/p_r_nodes.csv"
    )
    ...
    df = df[["index", "value"]]
    df.to_csv(project_path / "inputs/scenario/networks/p_r_capacities.csv", index=False)
    ...
    for asim_tod, sc_tod in time_period_lookup.items():
        ...
        my_project.import_matrix_from_csv(
            project_path / "inputs/scenario/networks/p_r_capacities.csv",
            "PARK_AND_RIDE_CAPACITY",
        )
