...
class EmmeProject:
    def __init__(self, filepath):
        ...
        self.zone_lookup = dict(
            (value, index)
            for index, value in enumerate(self.current_scenario.zone_numbers)
        )
    ...
    def matrix_calculator(self, spec, **kwargs):
    ...
    def create_new_mode(self, type, id, description):
        NAMESPACE = "inro.emme.data.network.mode.create_mode"
        create_mode = self.m.tool(NAMESPACE)
        create_mode(mode_type=type, mode_id=id, mode_description=description)
        compute_matrix = self.m.tool(NAMESPACE)
