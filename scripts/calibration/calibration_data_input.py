...
def get_households_data(uncloned=True, col_list=None):
    ...
    model = pd.read_parquet(
        config["p_model_households"], columns=model_cols
    ).reset_index()
    ...
    if uncloned:
        survey = (
            pd.read_csv(config["p_survey_households_uncloned"], usecols=survey_cols)
            .groupby("hhid_elmer")
            .first()
            .reset_index()
        )  # remove duplicates
...
def get_persons_data(uncloned=True, col_list=None):
    ...
    if uncloned:
        survey = (
            pd.read_csv(config["p_survey_persons_uncloned"], usecols=survey_cols)
            .groupby("person_id_elmer_original")
            .first()
            .reset_index()
        )  # remove duplicates
    else:
...
def get_tours_data(uncloned=True, col_list=None):
    ...
    if uncloned:
        trip_data = pd.read_csv(
            config["p_survey_trips_uncloned"],
            usecols=["tour_id", "trip_weight_2017_2019"],
        )
    else:
        trip_data = pd.read_csv(
            config["p_survey_trips"], usecols=["tour_id", "trip_weight_2017_2019"]
        )
    tour_weights = (
        trip_data.groupby(["tour_id"], group_keys=False)["trip_weight_2017_2019"]
        .mean()
        .reset_index()
    )
    ...
    if uncloned:
        survey = (
            pd.read_csv(config["p_survey_tours_uncloned"], usecols=survey_cols)
            .merge(tour_weights, how="left", on="tour_id")
            .reset_index()
        )
    else:
        survey = (
            pd.read_csv(config["p_survey_tours"], usecols=survey_cols)
            .merge(tour_weights, how="left", on="tour_id")
            .reset_index()
        )
    survey["source"] = "survey data"
...
def get_trips_data(uncloned=True, col_list=None):
    ...
    if uncloned:
        survey = pd.read_csv(
            config["p_survey_trips_uncloned"], usecols=survey_cols
        ).reset_index()
    else:
        survey = pd.read_csv(
            config["p_survey_trips"], usecols=survey_cols
        ).reset_index()
    survey["source"] = "survey data"
