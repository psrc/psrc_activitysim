...
if convert_survey:
    ...
    person[['person_id','person_id_original']].to_csv(os.path.join(output_dir,'person_id_mapping.csv'))
    person['person_id'] = person['person_id'].astype('int32')    

    # Set new person ID on all other files
    trip = trip.merge(person[['person_id','person_id_original']], left_on='person_id', right_on='person_id_original', how='left')
    trip.drop(['person_id_x','person_id_original'], axis=1, inplace=True)
    trip.rename(columns={'person_id_y': 'person_id'}, inplace=True)

    tour = tour.merge(person[['person_id','person_id_original']], left_on='person_id', right_on='person_id_original', how='left')
    tour.drop(['person_id_x','person_id_original'], axis=1, inplace=True)
    tour.rename(columns={'person_id_y': 'person_id'}, inplace=True)

    joint_tour_participants = joint_tour_participants.merge(person[['person_id','person_id_original']], left_on='person_id', right_on='person_id_original', how='left')
    ...
    # Get indicator if a person takes a joint tour
    # FIXME: this still fails joint tour frequency estimation if a household has 2 joint tours
    # with two different primary participants; fix by settings person that takes multiple joint tours
    # as primary participant 
    person['makes_joint_tour'] = 999
    person.loc[person['person_id'].isin(joint_tour_participants['person_id']), 'makes_joint_tour'] = 1
    person = person.sort_values(['household_id','makes_joint_tour'])
